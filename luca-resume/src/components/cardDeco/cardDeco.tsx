...
type CardDecoProps = PropsWithChildren<HTMLAttributes<HTMLDivElement> & CardDecoBaseProps>;

function CardDeco({children, className, percent = 100, ...rest}: CardDecoProps) {
  const [childRef, {height, width}] = useMeasure()
  const classNameContainer = `carddeco-container ${className || ''}`

  const childClone = cloneElement(children as React.ReactElement, {ref: childRef})
