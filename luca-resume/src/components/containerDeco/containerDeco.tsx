...
const ContainerDecoWithRef = forwardRef<HTMLDivElement, ContainerDecoProps>(
  (props, ref) => {
    const { children, className, ...rest } = props;

    const subComponents: ContainerDecoSubComponents = useSubComponents(
      children,
      ["Icon", "Content"]
    );
    ...
    return (
      <Container ref={ref} className={containerClassNames} {...rest}>
        <Row>
          {subComponents?.Icon && (
            <Col className="col-auto">{subComponents?.Icon}</Col>
          )}
